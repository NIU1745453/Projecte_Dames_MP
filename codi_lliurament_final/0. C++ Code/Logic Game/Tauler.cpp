...
// En Tauler.cpp
bool Tauler::esFitxaDelColor(const Posicio& pos, ColorFitxa color) const
{
	if (pos.getFila() >= 0 && pos.getFila() < N_FILES && pos.getColumna() >= 0 && pos.getColumna() < N_COLUMNES)
		return m_tauler[pos.getFila()][pos.getColumna()].getColor() == color;
...
{
	if (origen.getFila() >= 0 && origen.getFila() < N_FILES && origen.getColumna() >= 0 && origen.getColumna() < N_COLUMNES)
	{
		for (int i = 0; i < m_tauler[origen.getFila()][origen.getColumna()].getnValidas(); i++)
		{
			Moviment mov = m_tauler[origen.getFila()][origen.getColumna()].getMoviments(i);
			if (mov.getUltimaPosicion() == desti)
				return true;
		}
	}
	return false;
...
void Tauler::actualitzaMovimentsValids(const Posicio& pos)
{
	// Limpiar movimientos válidos previos
	netejaMovimentsValids();

	// Actualizar movimientos válidos para la posición seleccionada
	if (pos.getFila() >= 0 && pos.getFila() < N_FILES &&pos.getColumna() >= 0 && pos.getColumna() < N_COLUMNES)
		actualitzaMovimentsValids(); // Esta función ya está implementada en Tauler.cpp
}
...
void Tauler::netejaMovimentsValids()
{
	for (int i = 0; i < N_FILES; i++)
	{
		for (int j = 0; j < N_COLUMNES; j++)
			m_tauler[i][j].setnValidas(0);
	}
}

vector<Posicio> Tauler::getPosicionsValides(const Posicio& origen) const
{
	...
	if (origen.getFila() >= 0 && origen.getFila() < N_FILES && origen.getColumna() >= 0 && origen.getColumna() < N_COLUMNES)
	{
		...
		{
			Moviment mov = m_tauler[origen.getFila()][origen.getColumna()].getMoviments(i);
			posicions.push_back(mov.getUltimaPosicion());
		}
	}
	return posicions;
}

bool Tauler::jocAcabat() const
{
	...
	for (int i = 0; i < N_FILES; i++)
	{
		for (int j = 0; j < N_COLUMNES; j++)
		{
			if (m_tauler[i][j].getColor() == COLOR_BLANC)
				blanques++;
			else if (m_tauler[i][j].getColor() == COLOR_NEGRE)
				negres++;
		}
	}
	return (blanques == 0 || negres == 0);
}

ColorFitxa Tauler::getGuanyador() const
{
	int blanques = 0, negres = 0;
	for (int i = 0; i < N_FILES; i++)
	{
			...
			if (m_tauler[i][j].getTipus() != TIPUS_EMPTY)
			{
				...
				if (m_tauler[i][j].getColor() == COLOR_BLANC)
				{
					if (m_tauler[i][j].getTipus() == TIPUS_DAMA)
						sprite = GRAFIC_DAMA_BLANCA;
					else
						sprite = GRAFIC_FITXA_BLANCA;
				}
				else
				{
					if (m_tauler[i][j].getTipus() == TIPUS_DAMA)
						sprite = GRAFIC_DAMA_NEGRA;
					else
						sprite = GRAFIC_FITXA_NEGRA;
				}
